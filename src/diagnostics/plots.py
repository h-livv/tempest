...
from src.diagnostics.validation_renderers import ValidationRendererRegistry
...
class TempestPlotter:
    ...
    def plot_validation(
        ...
        *,
        ...
        if grid is not None and u_numerical is not None and u_analytical is not None:
            renderer = ValidationRendererRegistry.resolve(grid)
            if renderer:
                renderer.render(
                    self, grid, u_numerical, u_analytical, eq_name, solver_name, run_id, time_history_df, raw_tensor_data
                )
            else:
                print(f"No validation renderer found for grid with ndim={grid.ndim}")
