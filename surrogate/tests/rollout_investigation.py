...
from ml.core.data import load_combined_training_data, load_data, find_validation_data
from ml.core.train import train
from ml.core.eval import predict, compute_metrics, rollout, relative_l2, ROLLOUT_PAIR_INDEX
from ml.diagnostics.plots import PLOT_STYLE, spectrum_magnitudes
from ml.experiments.spectral.run import TRAIN_ICS, N_MODES, TRAIN_FRACTION, RANDOM_SEED, BATCH_SIZE, EPOCHS, LEARNING_RATE, USE_LINEAR
...
def make_output_dir() -> Path:
    ...
    output_dir = Path(__file__).resolve().parent / "outputs" / f"rollout_investigation_{timestamp}_{run_id}"
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir


def train_model(device: torch.device) -> tuple[nn.Module, dict[str, Any]]:
    """Train a single model with existing hyperparameters."""
    train_data = load_combined_training_data(TRAIN_ICS)
    nx = train_data["inputs"].shape[1]
    n_modes = min(N_MODES, nx // 2 + 1)
    ...
    print(f"Training model: nx={nx}, n_modes={n_modes}, epochs={EPOCHS}")
    history = train(
        model=model,
...
def trace_forward(model: SpectralNet, field: np.ndarray, device: torch.device) -> dict[str, np.ndarray]:
...
def step01_one_step(
    ...
    sample_index = ROLLOUT_PAIR_INDEX
    x_in = data["inputs"][sample_index]
    y_true = data["targets"][sample_index]
    y_pred = predict(model, torch.from_numpy(x_in).unsqueeze(0), device)[0]
    y_rollout = rollout(model, data["series"][sample_index], 1, device)[0]
    abs_err = np.abs(y_pred - y_true)
    metrics = compute_metrics(y_pred[None], y_true[None])
...
def step02_fft_pipeline(
    ...
    metrics = {
        "numpy_roundtrip_rel_l2": relative_l2(recon_np_full, field),
        "torch_roundtrip_rel_l2": relative_l2(recon_t_full, field),
        "numpy_torch_coeff_max_diff": float(np.max(np.abs(coeff_np - coeff_t.squeeze(0).cpu().numpy()))),
        "truncation_rel_l2": relative_l2(recon_trunc, field),
        "model_fft_path_rel_l2": relative_l2(recon_model_pre_linear, field),
        "n_modes": n_modes,
        "nx": nx,
    }
...
def step03_spectral_evolution(
    ...
    metrics = {
        "mag_rel_l2_low": relative_l2(mag_pred[low_band], mag_true[low_band]),
        "mag_rel_l2_high": relative_l2(mag_pred[high_band], mag_true[high_band]),
        "phase_mae_low": float(
        ...
        "complex_diff_rel_l2": relative_l2(coeff_diff, fft_true[:n_plot]),
    }
...
def rollout_diagnostics(
    ...
    prediction = rollout(model, series[pair_index], n_max, device)
        ...
        per_step.append(
            {
                ...
                "relative_l2": relative_l2(pred, ref),
    ...
    for horizon in ROLLOUT_HORIZONS:
        h = min(horizon, n_max)
        pred_h = rollout(model, series[pair_index], h, device)
        ref_h = reference[:h]
            ...
            horizon_metrics[str(h)].append(
                {
                    ...
                    "t_in": float(times[pair_index + step]),
                    "t_out": float(times[pair_index + step + 1]),
                    "relative_l2": relative_l2(pred_h[step], ref_h[step]),
    ...
    pred_spec = spectrum_magnitudes(prediction)[:, :MAX_PLOT_MODES]
    ref_spec = spectrum_magnitudes(reference)[:, :MAX_PLOT_MODES]
...
def step07_exposure_bias(
    ...
    ar_pred = rollout(model, series[pair_index], n_steps, device)
    tf_pred = teacher_forcing_rollout(model, series[pair_index : pair_index + n_steps], n_steps, device)

    ar_errors = [relative_l2(ar_pred[i], reference[i]) for i in range(n_steps)]
    tf_errors = [relative_l2(tf_pred[i], reference[i]) for i in range(n_steps)]
...
def step08_data_representation(
    ...
    traced = trace_forward(model, field, device)
    direct_pred = predict(model, torch.from_numpy(field).unsqueeze(0), device)[0]
...
def step09_translation_consistency(
    ...
    gaussian_data = load_data(find_validation_data("gaussian"))
    x = gaussian_data["x"]
    ...
    for idx, offset in enumerate(TRANSLATION_OFFSETS):
        ...
        pred = predict(model, torch.from_numpy(shifted).unsqueeze(0), device)[0]
        ...
        rel_l2 = relative_l2(pred, target_shifted)
...
Rollout pair index: `{ROLLOUT_PAIR_INDEX}` (t_in={t_in:.4f} → t_out={t_out:.4f})
...
- Shared pair index {ROLLOUT_PAIR_INDEX}: t_in={t_in:.4f}, t_out={t_out:.4f}
...
def main() -> None:
    ...
    data = load_data(find_validation_data(INVESTIGATION_IC))
    data_path = find_validation_data(INVESTIGATION_IC)
    ...
    sample_field = data["inputs"][ROLLOUT_PAIR_INDEX]
